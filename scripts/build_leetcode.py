#!/usr/bin/env python3
"""
build_leetcode_heatmap.py
Generates the leetcode.svg contribution heatmap widget.
Supports themes and includes custom data attributes for future snake overlay animation.
"""
...
SVG_TEMPLATE = Template("""<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 740 170" width="740" height="170">
  <defs>
    <style>
      ...
      .cell-active:hover { transform: scale(1.2); transform-origin: center; filter: brightness(1.2) drop-shadow(0 0 3px $accent); }
  ...
  <a href="https://leetcode.com/u/$username" target="_blank">
    ...
    <g>
      ...
      <text x="370" y="20" text-anchor="middle" class="header-text">$username@leetcode-heatmap:~</text>
    ...
    <g transform="translate(15, 152)">
      ...
      <text x="0" y="0" class="stats-text">$total_submissions submissions in the last year</text>
...
def build_leetcode_heatmap(theme_name):
    # Select theme
    ...
    if os.path.exists(leetcode_path):
        ...
        lc = {"username": "Guest", "submission_calendar": {}}

    username = lc.get("username", "Guest")

    # Map submission calendar to date objects
    leetcode_dates = {}
    for timestamp_str, count in lc.get("submission_calendar", {}).items():
        try:
            timestamp = int(timestamp_str)
            # Convert timestamp to date
            date_obj = datetime.datetime.fromtimestamp(timestamp, datetime.timezone.utc).date()
            leetcode_dates[date_obj] = leetcode_dates.get(date_obj, 0) + count
        except ValueError:
            continue
    ...
    for day_idx in range(371):
        ...
        # Determine submission count and level
        if current_date > today:
            ...
            count = leetcode_dates.get(current_date, 0)
    ...
    output_path = os.path.join(output_dir, "leetcode.svg")
    with open(output_path, "w", encoding="utf-8") as f:
        f.write(svg_content)

    print(f"Successfully generated leetcode.svg in {output_path} using theme: {theme_name}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Build LeetCode Heatmap SVG.")
    parser.add_argument(
        "--theme",
        default="matrix-green",
        choices=["matrix-green", "cyber-purple", "github-dark"],
        help="Color theme for the heatmap"
    )
    args = parser.parse_args()
    build_leetcode_heatmap(args.theme)
